...
/*
* Compares to another VariableManager to make sure they have the same variable names in the same the same positions.
...
VariableManager::hasSameVars(VariableManager* other)
{
	...
	for(map<string,int>::iterator aIter=varNameIDMap.begin();aIter!=varNameIDMap.end();aIter++)
	{
		...
		{
			cerr << "Incompatible variable name found at varID " << vId <<": first var " << name << ", test var " << otherVar->getName() << endl;
			success=false;
		}
	}
	return success;
