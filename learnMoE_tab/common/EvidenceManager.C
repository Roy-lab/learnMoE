...
* Reads columns as dimensions and rows as variables (genes).
* Sample names (columns) get stored in sampleNames.
*/
Error::ErrorCode
EvidenceManager::loadEvidenceFromFile_SimpleRows(const char* inFName)
{
	ifstream inFile(inFName);
	string buffstr;
	char* buffer=NULL;
	...
	while(inFile.good())
	{
		...
		if(lineNo==0)
		{
			//Reading in the header names -- these are samples
			char* tok=strtok(buffer,"\t");
			int tokCnt=0;
			while(tok!=NULL)
			{
		...
		{
			// each row is a variable (gene) with some number of values (samples)
			//All the evidences for each variable are stored in a map, indexed by the varId
			//EMAP* evidMap=new EMAP;
			char* tok=strtok(buffer,"\t");
			int tokCnt=0;
			...
			while(tok!=NULL)
			{
				if(tokCnt==0)
				{
					Error::ErrorCode eCode=vMgr->addVariable(tok,lineNo-1);
					if(eCode!=Error::SUCCESS)
					{
						cout <<"unable to add variable! "<< endl;
						return eCode;
				...
				{
					EMAP* evidMap=evidenceSet[tokCnt-1]; // each column is a "sample"
					Evidence* evid=new Evidence; // one value

					Variable* var=vMgr->getVariableAt(lineNo-1);
					if (var==NULL)
					{
