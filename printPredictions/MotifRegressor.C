...
// Don't use an RNG to do CV splitting
int 
MotifRegressor::learnMoE_CrossValidation(int neededfoldCnt)
{
	...
	r=gsl_rng_alloc(gsl_rng_default);
	int randseed=getpid(); // this is used for something else
	//int randseed=16951;
	cout <<"randseed: " << randseed << endl;
	gsl_rng_set(r,randseed);
	initVarsWithPredictors();
	...
	for(int i=0;i<foldCnt;i++)
	{
		...
		// DC added printing of true/predicted expression
		// and of overall CC for test data
		char foldCC[1024];
		sprintf(foldCC,"%s/fold%d/cc_test.txt",outputDir, i);
		ofstream ccf(foldCC);

		char expFName[1024];
		sprintf(expFName,"%s/fold%d/pred_test.txt",outputDir, i);
		ofstream outf(expFName);

		double testLL=getTestDataLikelihood(unpenalizedLL, outf, ccf);
...
MotifRegressor::predictTestExpression()
{
	...
	for(map<int,int>::iterator vIter=varsWithPredictors_Test.begin();vIter!=varsWithPredictors_Test.end();vIter++)
	{
		...
		// print one line per evidence value
		// name \t true_i \t moduleID \t predicted_i
		for(int i=0;i<dim;i++)
		{
...
/**
* Computes test data likelihood
* DC added:
* For each gene, predicts expression as mean of most likely module assignment.
* Prints predicted expression and module assignment to desired files.
* Outf: prints true/predicted results
* overallf: Prints the overall Pearson corr for all predicted values.
*/
double
MotifRegressor::getTestDataLikelihood(double& unpenalized, ofstream& outf, ofstream& overallf)
{
	VSET& varSet=varMgr->getVariableSet();
	double ll=0;
	int dim=evMgr->getNumberOfEvidences();
	...
	// per module true/predicted
	map<int,vector<double> > perModuleTrue;
	map<int,vector<double> > perModulePredicted;
	vector<double> allTrue;
	vector<double> allPredicted;
	...
	for(map<int,int>::iterator vIter=varsWithPredictors_Test.begin();vIter!=varsWithPredictors_Test.end();vIter++)
	{
		...
		// compute likelihood from each expert
		for(map<int,Expert*>::iterator eIter=expertSet.begin();eIter!=expertSet.end();eIter++)
		{
		...
		int bestID; // get the most likely expert for this gene
		double maxProb=0; // start with a small number
		...
		for(map<int,double>::iterator dIter=mixWts.begin();dIter!=mixWts.end();dIter++)
		{
			...
			double thisprob=mixOutProbs[dIter->first];
			if (thisprob > maxProb)
			{
				maxProb=thisprob;
				bestID=dIter->first;
		...
		// predict expression from the best expert
		Expert* e=expertSet[bestID];
		Matrix* mean=e->getMean();
		// figure out the gene name for printing to file DC Jan 2018
		...
		for(int i=0;i<dim;i++)
		{
			EMAP* evidMap=evMgr->getEvidenceAt(i);
			Evidence* evid=(*evidMap)[vIter->first];
			outf << name << "\t" << evid->getEvidVal() << "\t" << bestID << "\t" << mean->getValue(0,i) << endl;

			perModulePredicted[bestID].push_back(mean->getValue(0,i));
			perModuleTrue[bestID].push_back(evid->getEvidVal());
	...
	overallf << "AvgPerModule" << "\t" << totalCorr/perModuleTrue.size() << endl;
	double cc=d.computeCC(allTrue, allPredicted);
	overallf << "Overall" << "\t" << cc << endl;
...
MotifRegressor::showMoE()
{
	...
	for(map<int,Expert*>::iterator aIter=expertSet.begin();aIter!=expertSet.end();aIter++)
	{
...
int
MotifRegressor::getFeatureEnrichment(Expert* e,INTDBLMAP& featEnrich)
{
	VSET& varSet=varMgr->getVariableSet();
...
/**
* Performs the CV split using a random seed.
*/ 
int
MotifRegressor::split(int fId, int seed)
{
	cerr << "DOES NOT WORK" << endl;
	return 1;
	...
	gsl_rng* randnum = gsl_rng_alloc(gsl_rng_default);
	gsl_rng_set(randnum, seed);

	// generate random ID order (same as other folds due to same seed)
	vector<int> dataOrder;
	populateRandIntegers(dataOrder, varsWithPredictors.size(), randnum);

	// we'll step through all folds to make sure the order is the same
	int testBegin=0;
	int foldSize=(int)(varsWithPredictors.size()/foldCnt);
	for (int f=0; f<foldCnt; f++)
	{
		if (fId != f)
		{
			testBegin=testBegin+foldSize;
			continue; // We actually aren't working with this fold
		}

		int testEnd=testBegin+foldSize;
		for (int i =0; i< varsWithPredictors.size(); i++)
		{
			int randId=dataOrder[i];
			if (i>= testBegin && i<testEnd)
			{
				varsWithPredictors_Test[randId]=varsWithPredictors[randId];
			}
			else
			{
				varsWithPredictors_Train[randId]=varsWithPredictors[randId];
				Variable* var=varSet[randId];
				varsWithPredictors_Train_NameID[var->getName()]=randId;
				matIdvIdMap[i]=randId;
			}
		}
		testBegin=testEnd; // skip ahead
	}

	gsl_rng_free(randnum);
...
/**
* Populates a vector of random integers using a pre-seeded RNG.
* DC Jan 2018
*/
int 
MotifRegressor::populateRandIntegers(vector<int>& randInds,int size,gsl_rng* r)
{
	cerr << " MAY NOT WORK " << endl;
	return 1; 
...
int 
MotifRegressor::estimateMeanCov(vector<int>& randIndex,int startInd,int endInd, Expert* e)
{
	int dim=evMgr->getNumberOfEvidences();
	Matrix* mean=new Matrix(1,dim);
...
int 
MotifRegressor::estimateGlobalCov(Expert* e)
{
	int dim=evMgr->getNumberOfEvidences();
	Matrix mean(1,dim);
	for(int i=0;i<dim;i++)
...
int 
MotifRegressor::estimateMean(vector<int>& randIndex,int startInd,int endInd, Expert* e)
{
	int dim=evMgr->getNumberOfEvidences();
	Matrix* mean=new Matrix(1,dim);
	for(int i=0;i<dim;i++)
...
int 
MotifRegressor::estimateMeanCov(int expertid, Expert* e)
{
	int dim=evMgr->getNumberOfEvidences();
	Matrix* mean=e->getMean();
	double prior=0;
...
int 
MotifRegressor::setInitFeatureWeights(Expert* e)
{
...
MotifRegressor::assignGenesToExperts()
{
	...
	for(map<int,map<int,double>*>::iterator dIter=gammas.begin();dIter!=gammas.end();dIter++)
	{
		...
		for(map<int,double>::iterator gIter=gamma_d->begin();gIter!=gamma_d->end();gIter++)
		{
			...
			if(gIter->second>0.8)
			{
				Expert* e=expertSet[gIter->first];
